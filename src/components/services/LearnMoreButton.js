...
const LearnMoreButton = ({ isDark, isGreen }) => {
  return (
    <button className={`for_button ${isDark ? 'dark' : ''} ${isGreen ? 'green' : ''}`} style={{display: "flex", alignItems: "center", gap: "20px"}}>
      <svg
        width="41"
        height="41"
        viewBox="0 0 41 41"
      ...
      >
        <circle
          ...
          cy="20.5"
          r="20.5"
          fill={isGreen ? '#000' : isDark ? '#fff' : '#000'}
        />
        <path
          ...
          fill={isGreen ? '#B9FF66' : isDark ? '#000' : '#B9FF66'}
        />
      </svg>
        <span>
            Learn more
